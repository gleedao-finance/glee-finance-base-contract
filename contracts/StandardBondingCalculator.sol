...
contract GLFIBondingCalculator is IBondingCalculator {
  ...
  function markdown(address _pair) external view returns (uint256) {
    ...
    uint256 reserve;
    if (IUniswapV2Pair(_pair).token0() == address(GLFI)) {
      reserve = reserve1;
    } else {
      require(
        IUniswapV2Pair(_pair).token1() == address(GLFI),
        'not a GLFI lp pair'
      );
      reserve = reserve0;
    }
    return reserve.mul(2 * (10** GLFI.decimals())).div(getTotalValue(_pair));
  }
}
